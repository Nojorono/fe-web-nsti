  ...
  actions: {
    async fetchSearch({ commit }, payload) {
      ...
      */
      ...
      try {
        const { data } = await axiosInstance.post('searchbar/search', { ...payload })
        return data
      } catch (e) {
        throw e
      }
    ...
    async fetchMediaDetail({ commit }, id) {
      ...
      try {
        const { data } = await axiosInstance.get(`media/detail/${id}`)
    ...
    async fetchAllProducts({ commit }, payload) {
      try {
        const { data } = await axiosInstance.get(`product/readAll`)
